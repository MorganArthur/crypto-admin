...
import { listFiles, listStrategies, runFuturesBacktest } from "../api";
...
const FuturesBacktestPanel: React.FC = () => {
  ...
  useEffect(() => {
    ...
    listStrategies()
      .then((res) => {
        // 只显示合约策略
        const futuresStrategies = res.data.strategies.filter(s => s.type === 'futures' || s.name.includes('futures'));
        setStrategies(futuresStrategies);
        if (futuresStrategies.length > 0) {
          const first = futuresStrategies[0];
  ...
  const handleRunBacktest = async () => {
    ...
    setResult(null);
    try {
      const res = await runFuturesBacktest({
        symbol,
        timeframe,
          ...
          <Space wrap>
            ...
            <div>
              ...
              <InputNumber
                ...
                max={125}
                value={leverage}
                onChange={(val) => setLeverage(val || 10)}
                style={{ width: 120 }}
                addonAfter="X"
              />
            </div>
...
export default FuturesBacktestPanel;